...
namespace tsl {
	...
	namespace style {
		...
		namespace color {
			...
			constexpr u16 ColorHandle      = 0xF555;    ///< Scrollbar handle color (RGBA4444)
		}
	}
	...
	namespace gfx {
		...
		public:
			...
			*/
			 ...
			 * @param color Color
			 */
			inline void drawDashedLine(s16 x0, s16 y0, s16 x1, s16 y1, s16 line_width, s16 space_width, Color color) {
			...
			inline void endFrame() {
				...
				if (R_SUCCEEDED(rc)) {
					...
					if (mode == ApmPerformanceMode_Boost) {
						...
						if (R_SUCCEEDED(rc)) {
							u32 underscanValue = tv_settings.underscan;
							if (underscanValue != last_underscan_value) {
								updateUnderscan(underscanValue);
							}
						}
						else {
							updateUnderscan(0);
	...
	namespace elm {
		...
		public:
			...
			virtual inline s32 getTopBound()    final { return this->getY(); }
			virtual inline s32 getLeftBound()   final { return this->getX(); }
			virtual inline s32 getRightBound()  final { return this->getX() + this->getWidth(); }
			virtual inline s32 getBottomBound() final { return this->getY() + this->getHeight(); }
		...
		private:
			...
			// Storage widened from u16 to s32 so that scrolled-off-top list items
			// can have negative Y coordinates without wrap-around. u16 → s32 is a
		...
		public:
			...
			virtual void draw(gfx::Renderer *renderer) override {
				renderer->fillScreen(a(backgroundColor));
			...
			virtual void layout(u16 parentX, u16 parentY, u16 parentWidth, u16 parentHeight) override {
				...
				if (this->m_contentElement != nullptr) {
                    this->m_contentElement->setBoundaries(parentX + 35, parentY + 140, parentWidth - 85, parentHeight - 73 - 145); // CUSTOM MODIFICATION
		...
		public:
			 ...
			 * @brief Constructor
			 * 
			 * @param text  Initial description text (left-hand label)
			 * @param value Optional right-hand value text. Empty by default; pass a
			 *              non-empty string to construct the item with its value
		 ...
		 *
		 * @note This implementation uses pixel-based smooth scrolling. When the total
		 *       height of all items exceeds the visible area, a scrollbar is drawn
		 *       on the right side and the scroll position animates smoothly when
		 *       focus changes. The legacy \p entriesShown constructor argument is
		 *       accepted for API compatibility but is no longer used to drive
		 *       scrolling (the visible region is derived from the parent's
		...
		public:
			 ...
			 * @brief Constructor
			 * 
			 * @param entriesShown (legacy/unused) Kept for API compatibility with
			 *        older code that constructed a List with an explicit count.
			 */
			...
			virtual void draw(gfx::Renderer *renderer) override {
				// Asymmetric scissor:
				//   - Extend 4 px on left, right, and top so the focused item's
				//     highlight border (which Element::drawHighlight paints 4 px
				//     outside the item bounds on every side) is fully visible.
				//   - Pull the bottom in by 1 px so the bottom separator drawn at
				//     y = item.Y + item.H is clipped when an item is sitting
				//     exactly at the list bottom (i.e. fully scrolled). The
				//     scissor check is `y > bounds.y + bounds.h`, so setting the
				//     bottom to list.Y + list.H - 1 rejects y = list.Y + list.H.
				constexpr s32 HL_EXTEND = 4;   // highlight border extent
				renderer->enableScissoring(
					this->getLeftBound() - (2*HL_EXTEND),
					this->getTopBound() - (3*HL_EXTEND),
					this->getWidth()  + (4*HL_EXTEND),
					this->getHeight() + (4*HL_EXTEND) - 1
				);

				for (auto &entry : this->m_items) {
					// Only render entries whose vertical extent intersects the visible area.
					if (entry.element->getBottomBound() > this->getTopBound() &&
						entry.element->getTopBound()    < this->getBottomBound()) {
				...
				// Draw the scrollbar when content overflows the visible region.
				if (this->m_listHeight > this->getHeight()) {
					const s32 trackTop    = this->getY();
					const s32 trackHeight = this->getHeight();
					float scrollbarHeight = static_cast<float>(trackHeight) * static_cast<float>(trackHeight) / static_cast<float>(this->m_listHeight);
					if (scrollbarHeight < 20.0f) scrollbarHeight = 20.0f;

					float maxOffset = static_cast<float>(this->m_listHeight - trackHeight);
					if (maxOffset < 1.0f) maxOffset = 1.0f;

					float scrollRange  = static_cast<float>(trackHeight) - scrollbarHeight;
					float scrollbarOffset = (this->m_offset / maxOffset) * scrollRange;
					if (scrollbarOffset < 0)          scrollbarOffset = 0;
					if (scrollbarOffset > scrollRange) scrollbarOffset = scrollRange;

					const s32 barX = this->getRightBound() + 10;
					const s32 barY = trackTop + static_cast<s32>(scrollbarOffset);
					const s32 barH = static_cast<s32>(scrollbarHeight);

					// Rounded-rect handle: middle rect + two filled circle caps.
					renderer->drawRect(barX,     barY + 2, 5, barH - 4, a(tsl::gfx::Color(tsl::style::color::ColorHandle)));
					renderer->drawCircle(barX + 2, barY + 2,         2, true, a(tsl::gfx::Color(tsl::style::color::ColorHandle)));
					renderer->drawCircle(barX + 2, barY + barH - 2,  2, true, a(tsl::gfx::Color(tsl::style::color::ColorHandle)));

					// Animate the scroll offset toward the target each frame.
					float prevOffset = this->m_offset;
					this->m_offset += (this->m_nextOffset - this->m_offset) * 0.1F;

					// Snap when within sub-pixel distance to avoid endless tiny redraws.
					if (std::fabs(this->m_nextOffset - this->m_offset) < 0.5F)
						this->m_offset = this->m_nextOffset;

					if (static_cast<s32>(prevOffset) != static_cast<s32>(this->m_offset))
						this->invalidate();
				}
			}

			virtual void layout(u16 parentX, u16 parentY, u16 parentWidth, u16 parentHeight) override {
				// Recompute total list height.
				this->m_listHeight = 0;
				for (auto &entry : this->m_items)
					this->m_listHeight += entry.height;

				// Place each item, accounting for the current scroll offset.
				s32 y = this->getY() - static_cast<s32>(this->m_offset);
				for (auto &entry : this->m_items) {
		...
		protected:
			...
			// Legacy member, kept only so its symbol stays available to any old
			// user code that may have read it. The new pixel-based offsets below
			// drive scrolling.
			u16 m_entriesShown = 5;

			// Smooth-scrolling state (pixel-based).
			float m_offset = 0.0f;       ///< Current animated scroll offset in pixels
			float m_nextOffset = 0.0f;   ///< Target scroll offset (jumped to on focus change)
			s32   m_listHeight = 0;      ///< Sum of all item heights

		private:
			/**
			 * @brief Recompute the target scroll offset so that the focused item
			 *        is comfortably inside the visible region.
			 */
			void updateScrollOffset() {
				...
				s32 focusedTop = 0;
				for (u16 i = 0; i < this->m_focusedIndex && i < this->m_items.size(); i++)
					focusedTop += this->m_items[i].height;

				s32 focusedHeight = (this->m_focusedIndex < this->m_items.size())
					? this->m_items[this->m_focusedIndex].height
					: 0;
				s32 focusedBottom = focusedTop + focusedHeight;

				const s32 viewTop    = static_cast<s32>(this->m_nextOffset);
				const s32 viewBottom = viewTop + this->getHeight();

				// Scroll just enough to make the focused item visible.
				if (focusedTop < viewTop) {
					this->m_nextOffset = static_cast<float>(focusedTop);
				}
				else if (focusedBottom > viewBottom) {
					this->m_nextOffset = static_cast<float>(focusedBottom - this->getHeight());
				}

				// Clamp.
				const s32 maxOffset = this->m_listHeight - this->getHeight();
				if (this->m_nextOffset < 0)
					this->m_nextOffset = 0;
				if (this->m_nextOffset > static_cast<float>(maxOffset))
					this->m_nextOffset = static_cast<float>(maxOffset);
			}
		};

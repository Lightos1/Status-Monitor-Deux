...
| `HISTORY_AVERAGE{historyName}` *(expression, not a command)*                        | Arithmetic mean of the ring buffer's current samples   |
...
- `matchLineHeight` — mirrors the `matchLineHeight` argument from the script. `true` means the renderer should clamp each line's height to exactly `fontSize` pixels (no natural font leading); `false` means use the font's own leading. The parser sets this directly from the script literal and never modifies it.
...
- `matchLineHeight` — same semantics as on `RenderCmdType::Text`. The host should perform its measurement under the same line-height mode it would use to draw the text, so the returned dimensions match what `TEXT` will actually produce. The measurement cache is keyed on `(text, fontSize, matchLineHeight)`, so `true` and `false` variants are cached independently.
...
This is the only callback kind where the host *writes back*. The host's font/renderer measures the string, fills `*outDims`, and returns. The parser caches the result keyed on `(text, fontSize, matchLineHeight)` so subsequent GET_DIMENSIONS in the same frame are free. The cache has a soft cap of 256 entries (drops the whole cache when exceeded — see `ClearDimsMeasureCache`).
...
**System**: `System_DisplayRefreshRate_int`, `System_IsDocked`, `System_KeysDown_int`, `System_KeysHeld_int`, `formattedKeyCombo` (string), `System_ClockHour_int`, `System_ClockMinute_int`, `System_ClockSecond_int`, `System_CalendarYear_int`, `System_CalendarMonth_int`, `System_CalendarDay_int`, `System_OverlayRenderingFrameTimeInNs_int`, `System_OverlayMemoryLeftInB_int`.
...
- `historyAvgScratches` — `std::unordered_map<std::string, std::unique_ptr<double>>`. One scratch `double` per history that is referenced by `HISTORY_AVERAGE{name}`. Allocated lazily during `EnsureVariable` (the prescan) when a `__havg_<histname>` identifier is first encountered. The te_variable table points at these doubles; `RefreshScratches` recomputes each one via `std::accumulate` over the matching ring buffer every frame.
...
- **Translate `HISTORY_AVERAGE{name}`** → `__havg_name` (a plain identifier). The `__havg_` prefix is chosen to be impossible to write by hand, preventing collisions with any author-defined name. An optional leading `$` inside the braces is stripped. `EnsureVariable` recognises the `__havg_*` prefix and allocates a backing scratch double from `Impl::historyAvgScratches`; `RefreshScratches` computes `std::accumulate` over the ring buffer and writes the result there each frame. See §6 for the full implementation.
...
| `test_history_average`        | `HISTORY_AVERAGE{name}` — inline, via VAR, all three element types, empty ring, ring wrap-around, cross-frame updates, arithmetic / `#if` / ternary use, Free+reload, `HISTORY_CLEAN`, `$` prefix |
...
**Adding a new `PreprocessExpr` identifier rewrite** (like `HISTORY_AVERAGE`):
1. Add a keyword-match block in `PreprocessExpr` that rewrites the `KEYWORD{arg}` form to a mangled plain identifier (e.g. `__prefix_arg`). Use a `__`-prefixed mangled name so authors can never collide with it by accident.
2. In `EnsureVariable`, add a branch before the `implicitZero` fallthrough that recognises the `__prefix_*` pattern, resolves it to the right backing storage (allocate lazily on first use via a new `Impl` map), and returns the double pointer.
3. In `Compile()`, push the mangled name + pointer into the te_variable table (park the name string in `implicitZero` for stable `c_str()` lifetime, same as resolution-array slot names).
4. In `RefreshScratches`, add a loop that recomputes the scratch value from whatever live data backs it.
5. In `Free()`, clear the new map alongside the rest.
...
- **`HISTORY_AVERAGE{name}` is a parse-time rewrite, not a function.** It is translated to `__havg_<name>` by `PreprocessExpr` and backed by a scratch double in `historyAvgScratches`. This means it works in every expression context — format args, VAR RHS, `#if`, ternaries, arithmetic — with no special-casing in the evaluator. The scratch is recomputed via `std::accumulate` in `RefreshScratches` each frame, so it always reflects the ring buffer as it stood after the last `HISTORY_UPDATE`. An empty ring yields `0.0`. Capacity-3 ring after 4 pushes holds the 3 most recent samples; the average is of those 3.
- **`historyAvgScratches` is populated lazily by the prescan.** If you reference `HISTORY_AVERAGE{foo}` and `foo` is not a declared `HISTORY`, `EnsureVariable` falls through to `implicitZero` and the expression compiles to `0.0` without error. This is intentional — the same graceful-degradation behaviour as unbound identifiers.

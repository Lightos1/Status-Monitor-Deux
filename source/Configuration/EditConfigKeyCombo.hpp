...
public:
	EditConfigKeyCombo(bool isMainCombo, std::string key, std::string value, std::string localName) {
		...
		if (value.length() > 0) m_value = value;
			...
			if (ultrahandCombo == true) m_value = "Ultrahand";
			else if (teslaCombo == true) m_value = "Tesla";
		}
	}
	...
	virtual tsl::elm::Element* createUI() override {
		...
		auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
			...
			auto [width, height] = renderer->drawString(m_value.c_str(), false, 0, fontsize2, fontsize2, renderer->a(0x0000));
